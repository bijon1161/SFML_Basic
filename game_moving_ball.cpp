...
int main()
{
	...
	Texture background;
		...
		if (!background.loadFromFile("Textures/sp2.jpg"))
			throw "Could not load cat.png!";
	
		bg.setTexture(background);
		//bg.setPosition(Vector2f(1280, 720));
		//bg.setScale(Vector2f(0.4f, 0.4f));
	...
	scoreText.setPosition(window.getSize().x / 2+420.f, 10.f);

	Text gameOverText;
	gameOverText.setFont(font);
	gameOverText.setCharacterSize(80);
	gameOverText.setFillColor(Color::White);
	gameOverText.setPosition(window.getSize().x / 2-185.f, window.getSize().y / 2-60.f);
	...
	hpText.setPosition(window.getSize().x / 2 + 55.f, 10.f);
	...
	while (window.isOpen())
	{
		...
		if (HP > 0 && status==0 )
		{
			...
			hpbar.setSize(Vector2f((float)HP * 20.f, 12.f));
			hpbar.setPosition(window.getSize().x / 2 + 420.f, 43.f);
		}
